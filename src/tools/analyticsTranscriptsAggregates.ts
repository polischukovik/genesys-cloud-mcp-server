...
import { z } from "zod";
import { transcriptAggregationQuerySchema } from "./utils/analyticsSchemas.js";
...
const paramsSchema = z.object({
  query: transcriptAggregationQuerySchema.describe(
    "TranscriptAggregationQuery payload for POST /api/v2/analytics/transcripts/aggregates/query.",
  ),
});
  ...
  createTool({
    ...
    call: async ({ query }) => {
      ...
      return successEnvelopeResult(response, {
        endpoint: "/api/v2/analytics/transcripts/aggregates/query",
      });
    },
  });
